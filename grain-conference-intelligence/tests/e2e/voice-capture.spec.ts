...
async function openCaptureForm(page: Page) {
  ...
  await expect(page.getByRole("heading", { name: "You planned to meet" })).toBeVisible({ timeout: 15_000 });
  await page.getByRole("button", { name: "Capture unplanned meeting" }).click();
  await expect(page.getByRole("heading", { name: "Save meeting" })).toBeVisible();
}
